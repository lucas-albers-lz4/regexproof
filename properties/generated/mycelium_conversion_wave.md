...
`sed -E` at the AllowedIPs last-octet substitution as at the call site — not
sed-for-bash-`=~`; GNU not consulted).
Not in `WAVE_CORPORA`. Not mixed into `OW-packages` / `OW-luci` / `AI-aidevops`.
`tests/conformance` was Gate 1 drop (no boundary).
Asked: **4** human contracts (ledger `properties_asked` 26 → 30).
...
| Site | Decision | Why |
|---|---|---|
| awg-issue last octet `([0-9]+)/` `:639` | **asked** shape 3 (slash-delimited UNSAT) | Capture identity → client address slot in live `awg0.conf`. Real delimiter is `/`. |
...
| tunnel `.[0-9]+/[0-9]+$` rewrite `:174` | skip | Substitution image sibling of asked last-octet capture. |
...
- 4 expected-UNSAT (3 shape 1 + 1 shape 3 capture identity).
...
Acceptance: harness runs with `--require-contract --require-ground-truth`; every property has `kind=` / `family=MY-mycelium`. Expected-UNSAT shape-3 fidelity is BusyBox differential fuzz (`ci-check-busybox-mycelium.py`).

## Stop vs next slice

**Wave 1 idiom slice done.** Do not re-ask sshd key-type prefix charset, AWG dialect-key charset, ALPN h2 line charset, or AllowedIPs last-octet `/` capture. Do not re-ask hostname / JSON `[^"]*` / digit-semicolon / brief t-ID.
...
**Do not:** packages/LuCI/aidevops re-open, Smith drain, `WAVE_CORPORA`, public filing without approval, another digit-no-semicolon alphabet, JSON `[^"]*`, hostname/IPv4/MAC/IPv6 charset, tautological key prefix-strips.
