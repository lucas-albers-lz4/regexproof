...
def _busybox_sed(stream: str) -> str:
    proc = subprocess.run(
        ["busybox", "sed", "-E", BRIEF_SED],
        input=stream,
        capture_output=True,
        text=True,
        timeout=30,
...
def _busybox_grep_o(stream: str) -> str:
    proc = subprocess.run(
        ["busybox", "grep", "-oE", ISSUE_GREP],
        input=stream,
        capture_output=True,
        text=True,
        timeout=30,
...
def _fuzz_brief_tid(n: int = 32, seed: int = 42) -> int:
    ...
    for i in range(n):
        ...
        if got != tid:
            ...
            return 2
    print(f"brief-tid differential fuzz: {n} t[0-9]+ ids identity-ok on BusyBox")
    return 0
