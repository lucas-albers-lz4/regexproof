...
from z3 import (Solver, unknown, sat, unsat,
                String, StringVal, InRe, Contains, Length, IndexOf, SubString,
                Range, Union, Concat, Star, Re)
...
def prop(name, declared_domain, expect_unsat=True, timeout_ms=30000):
    ...
    def deco(fn):
        REGISTRY[name] = {
            "fn": fn, "domain": declared_domain,
            "expect_unsat": expect_unsat, "timeout_ms": timeout_ms,
        }
        return fn
    return deco
...
def run_one(name, entry):
    ...
    if r == sat:
        m = s.model()
        for d in m.decls():
            print(f"    witness: {d.name()} = {repr(m[d])}")
    return ok
...
# ---------------------------------------------------------------------------
USERNAME_CLS = Union(Range('a', 'z'), Range('A', 'Z'), Range('0', '9'),
                     Re('_'), Re('-'))
USERNAME_RE = Concat(Union(Range('a', 'z'), Re('_')), Star(USERNAME_CLS))
DENY_LIST = ["root", "daemon", "ftp", "network", "nobody", "nogroup",
             "admin", "ubus", "sync"]
INJECTION_CHARS = [("space", " "), ("equals", "="), ("newline", "\n"),
                   ("tab", "\t"), ("semicolon", ";"), ("pipe", "|"),
                   ("dollar", "$"), ("backtick", "`"), ("ampersand", "&")]
...
for _name, _ch in INJECTION_CHARS:
    REGISTRY[f"P1-{_name}"] = {"fn": lambda ch=_ch: _p1(ch), "expect_unsat": True,
                               "timeout_ms": 30000,
...
@prop("P4-escape-image-newline",
      "same token alphabet contains no raw newline", expect_unsat=True)
def p4_nl():
    w = String('w')
    return [InRe(w, ESCAPE_TOKENS), Length(w) == 1], w == StringVal("\n")


@prop("P4-escape-image-del",
      "same token alphabet contains no raw DEL (0x7f)", expect_unsat=True)
def p4_del():
    w = String('w')
    return [InRe(w, ESCAPE_TOKENS), Length(w) == 1], w == StringVal("\x7f")


@prop("P4-nul-passthrough-demo",
...
# ---------------------------------------------------------------------------
WEAKENED_USERNAME_RE = Concat(Union(Range('a', 'z'), Re('_')),
                              Star(Union(USERNAME_CLS, Re('*'))))


@prop("P1-mutated-star",
      ...
      "mirror is not tracking the real regex.", expect_unsat=False)
def p1_mutated():
    u = String('u')
...
def main():
    ...
    if not args or "--all" in args:
        ...
        for n in sorted(REGISTRY):
            print(f"{n}  expect_unsat={REGISTRY[n]['expect_unsat']}  "
