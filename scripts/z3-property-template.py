...
alphabet = Union(Range('a', 'z'), Range('A', 'Z'), Range('0', '9'),
                 Re("."), Re("_"), Re("@"), Re("-"))
s = String('s')
for badchar, nm in [(" ", "space"), ("=", "equals"), ("\n", "newline"),
                    (";", "semicolon"), ("|", "pipe"), ("$", "dollar")]:
    check(f"alphabet excludes {nm}",
...
check("escaped-quote value: sed capture differs from true value (bug repro)",
      [first_quote > 0], Contains(v, StringVal('\\"')), expect_unsat=False)
...
for ch, nm in [("\t", "tab"), ("\n", "newline"), ("\x7f", "DEL")]:
    check(f"escaped output has no raw {nm}",
...
check("nul-passthrough demo (why the domain assumption is needed)",
      [InRe(w, Union(tokens, Re('\x00'))), Length(w) == 1],
      w == StringVal('\x00'), expect_unsat=False)
