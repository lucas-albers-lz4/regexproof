...
BASELINE = 121 / 875  # committed Phase 0 escape baseline ≈ 13.8% (batch 4 emptied 19 bad auto-NO-GOs)


def test_wilson_ci_escape_baseline():
    lo, hi = wilson_ci(121, 875)
    # Design pins ~[11.7%, 16.3%] for the 121/875 baseline.
...
def test_escape_fires_below_baseline():
    # 5/100 ≈ 5% vs 13.8% baseline → p ≈ 0.008 < 0.05 → FIRES (blocks scale).
...
def test_escape_continuity_correction_boundary():
    ...
    own committed baseline — k=3/n=50 vs BASELINE (121/875): uncorrected
    p≈0.0544 (just above 0.05) vs corrected p≈0.0809 (does NOT fire)."""
    t = two_proportion_test(k_window=3, n_window=50, baseline=BASELINE)
    assert t["fires"] is False
    assert t["p_value"] == pytest.approx(0.080942, abs=1e-6)  # corrected oracle
    # Sanity: without the correction the same input is just above 0.05 (0.054400649757842645).
    se = (0.06 - BASELINE) / ((BASELINE * (1.0 - BASELINE) / 50) ** 0.5)
    from regexproof.stats.intervals import _normal_cdf

    assert _normal_cdf(se) == pytest.approx(0.054400649757842645, abs=1e-6)
